...
`pc_sampling_gfx1151_nocwsr` branch contains an implementation of PC sampling on gfx1151 before the CWSR issue were fixed, where we make the trap handler a no-op and directly read information about the waves from SQ_IND. Whan the waves stall, the results can be different from the current approach.
