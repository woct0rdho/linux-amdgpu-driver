...
Tested with Ubuntu 26.04, Linux kernel 7.0 , nightly ROCm from TheRock as of 20260916. Use at your own risk.

`dkms` branch contains the conversion from source tree to dkms, a fix for a double free issue when failed to create pid kobj, and fix for failing to unload the dkms module.
...
Bonus: Thread tracing also seems to work, including multi-buffer.

## Installation

1. Clone this repo (use `--filter=blob:none` to save time) and checkout `pc_sampling_gfx1151` branch
2. `python3 setup_dkms_linked.py`
